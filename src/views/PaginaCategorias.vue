    ...
    <div class="ayuda">
      <div class="student__profile">
         <div class="content__welcome">
            <span class="welcome__student">CATEGORIAS</span>
          </div>
        </div>
        ...
        <div class="abajo">
            <div v-for="categoria in categorias">
              <div class="card"  v-if="filteredTareas(categoria.nombre).length>0" :key="categoria.idcategorias">
                <h1 >{{categoria.nombre}}</h1>
                <div class="EmpresasDestacadas" >
                    <div v-for="(data, index) in filteredTareas(categoria.nombre)" :key="index" class="company-cards-container" @click="navegarATareaDetalle(data)">
                      <PruebaTarea3 
            ...
            nuevoObjetoTarea:
{
    ...
    methods: {
      filteredTareas(nombreCategoria) {
        console.log("----------",this.todasTareas);
          return this.todasTareas.filter(tarea => tarea.b === nombreCategoria);
      },
